...
@section('script')
    <script>
        ...
        function paginate(
            ...
            prev = startPage -1;
            if(startPage > 1){
                ret += `    <li class="page-item"><a class="page-link active" href="{{url('/')}}/likedtracks/` + prev + `/20">&laquo</a></i>
                    `;
            }

            $.each(pages,function(i,li){
                if(currentPage === li){
                    ret += `    <li class="page-item"><a class="page-link active" href="{{url('/')}}/likedtracks/` + li + `/20">` + li + `</a></i>
                    `;
                }
                else{
                    ret += `    <li class="page-item"><a class="page-link" href="{{url('/')}}/likedtracks/` + li + `/20">` + li + `</a></i>
                    `;
                }
            });
            next = currentPage+1;
            if(endPage < totalPages){
                ret += `    <li class="page-item"><a class="page-link active" href="{{url('/')}}/likedtracks/` + next + `/20">&raquo</a></i>
                    `;
            }
            ret += `</ul></div></div>`;
            return ret;
        ...
        $(document).ready(function(){
            ...
            $.ajax({
                type: "GET",
                url: "{{url('/')}}/userlikes/track/" + page + "/" + perPage,
                success: function (response) {
                    ...
                    $.each(trackArray, function(i,data){
                                ...
                                <div class="col-12 artist-card compact">
                                    <div class="row">
                                            ...
                                            <div class="col-9 col-sm-10 col-xl-11 info-container compact">
                                                    ...
                                                    <span class=""><a href="{{url('/')}}/album/` + album.id + `">` + albumName + ` (` + releaseYear + `)</a></span><br>
...
@section('mainbody')
            <div class="col-md-12 toggle-bar"><div class="row"><div class="col-6 pt-1"><h3 id="tracks-heading" class="panel-heading">Liked Tracks</h3></div><div class="col-6"><em class="btn float-right icon dripicons-view-thumb" title="Full Size Panel View"></em><em class="btn float-right icon dripicons-view-list-large"  title="Compact View"></em></div></div>
