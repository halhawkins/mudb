...
@section('script')
    <script>
        ...
        function paginate(
            ...
            prev = startPage -1;
            if(startPage > 1){
                ret += `    <li class="page-item"><a class="page-link active" href="{{url('/')}}/likedalbums/` + prev + `/20">&laquo</a></i>
                    `;
            }

            $.each(pages,function(i,li){
                if(currentPage === li){
                    ret += `    <li class="page-item"><a class="page-link active" href="{{url('/')}}/likedalbums/` + li + `/20">` + li + `</a></i>
                    `;
                }
                else{
                    ret += `    <li class="page-item"><a class="page-link" href="{{url('/')}}/likedalbums/` + li + `/20">` + li + `</a></i>
                    `;
                }
            });
            next = currentPage+1;
            if(endPage < totalPages){
                ret += `    <li class="page-item"><a class="page-link active" href="{{url('/')}}/likedalbums/` + next + `/20">&raquo</a></i>
                    `;
            }
            ret += `</ul></div></div>`;
            return ret;
        ...
        $(document).ready(function(){
            ...
            $.ajax({
                type: "GET",
                url: "{{url('/')}}/userlikes/album/" + page + "/" + perPage,
                success: function (response) {

                    albums = response.albums;
                    totalAlbums = response.count;
                    albs = $("#albums");
                    $("#albums-heading").append(" (" + String((page-1)*perPage+1) + "-" + String((page-1)*20+response.albums.length) + " of " + totalAlbums + ")");
